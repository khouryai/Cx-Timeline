...
`CLAUDE.md` holds the rules — short enough to read before every change. This
file holds the reasons: what each rule prevents, what went wrong before it
existed, and the cases it has to cover. When a rule in `CLAUDE.md` seems wrong
for what you are doing, the section it links to here is where to find out
whether it is.

## Conventions

### Colours go through tokens, never raw hex
...
### The design language is shared with cx-portal
...
### Every document mutation goes through the store
...
### `getDoc()` is never mutated in place
...
### Hidden is a way back, not a way out
...
### A date window hides; the rest of the filter dims
...
### Baseline comparison draws five things, not one
...
### What actually happened is drawn, and it is what a baseline is compared against
...
### A note written on an object is shown on the timeline
...
### A ghost is packed, not painted over
...
### A P6 baseline is derived; a taken baseline is frozen
...
### P6 data is a register, not objects
...
### A pair of objects may carry several dependencies — one per type
...
### Selecting a bar marks what it is waiting on
...
### Say it once, in the status bar, without taking the keyboard
...
### Two windows of one install are one machine
...
### Waiting and taking over are not the only two options
...
### The pen borrows the name the calendar already knows
...
### `core/filestore.js` has two backends and one set of rules
...
### The desktop build has no backend for the plan, and may have one for the calendar
...
### The desktop app is fed by the deployment, not by reinstalling
...
### A download is the one action with no visible result, so it says so
...
### The resource calendar is a second module, not a second product
...
### A grid is the one heavy row, and it moves once
...
### A photograph goes up before the row, never after
...
### The digest carries no rate and no score
...
### The look-ahead now says who, and a Resource row is not an activity
...
### The sheet says who is away as well as who is on what, and none of it is scope
...
### `ABSENCE_KINDS` holds three facts about each kind, in one table
...
### PTO is a third reading, not a second store
...
### A name in a spreadsheet is matched exactly, corrected out loud, or reported — never guessed
...
### Unscheduled rows are hidden, and a heading is not exempt
...
### The key describes what is on screen, not the register
...
### The Changes list is about the weeks anybody can still act on
...
### Today is a column, not a hairline
...
### A member plans their own days, and takes them back
...
### A member reads the plan and never writes it
...
### Reports, Organisation and the Daily huddle are an administrator's, and the tabs say so
...
### A day is a list of tasks, not a task
...
### There is one place a day is planned
...
### When the sheet moves a task, the task moves
...
### Nothing is emailed from the application, and that is structural
...
### Adding somebody to the calendar never needs the SQL editor
...
### Plan data must never reach Supabase, and that is enforced rather than intended
...
### A blocked day is not an obstacle until somebody owns it
...
### Asking for leave is a member's; answering is an administrator's
...
### Two families of status, never averaged
...
### Retire is the first answer; Delete is the second, and Postgres decides which applies
...
### Evidence is append-only, in the database
...
### The look-ahead's window moving is not a change of scope
...
### A colour the legend does not know is never guessed
...
### What a colour is, is changed in Legend and nowhere else
...
### The workbook's own key is read once, into an empty register
...
### The location column is found the same way the date axis is, and the spelling is kept whether or not it resolves
...
### A read that started recording the location is not a read where everything moved
...
### The date axis is found, never configured
...
### White is not a highlight, and shading is not work
...
### A section heading is found structurally, not by its colour
...
### The calendar draws the snapshot, not the file
...
### A plan is revised, never edited
...
### The calendar's data can be taken out whole
...
### A SAR is recorded, filed and linked in that order, and each step can fail alone
...
### New user actions go in `ui/commands.js`
...
### The filter's text box holds a list, not a phrase
...
### Dropdown vocabularies are document data, not constants
...
### A duration is counted on a five-day week by default
...
### Dates are UTC-midnight milliseconds internally
...
### No timeline text is ever truncated, ellipsised or clamped
...
### The calendar is a second bundle, fetched the first time it is opened

`ui/rc.js` and everything only it reaches are built into `calendar.bundle.js`;
the main bundle carries `ui/calendar_loader.js`, which injects that script on
first use and resolves the entry out of `window.__CX_MODULES`. A timeline user
never downloads the 400 kB of calendar they may never open. Three places know
the file name and all three are checked: `tools/build.js` refuses a main bundle
that imports `ui/rc.js` statically (it would silently pull the whole calendar
back in), `tools/dist.js` fingerprints `calendar.<hash>.js` and rewrites the
`CALENDAR_BUNDLE` literal in the main bundle *before* hashing it, and
`tools/desktop.js` concatenates both bundles into one payload script, because an
installed shell's loader knows exactly one file and must still get the
calendar. A load that fails emits `EV.CALENDAR_FAILED`, says so in a toast, and
can be retried — it never takes the timeline with it. The build also parses
every bundle it writes (`assertParses`), so a file no browser can load cannot
be committed.

### The folder's pure rules live in a leaf, and agree with the Rust

`core/folder_rules.js` holds the names, the digest and `penHolder()` — no I/O,
no timers, no state — so `tools/test_folder_rules.js` checks them in Node in a
fraction of a second. `core/filestore.js` imports them; `src-tauri/src/plan.rs`
makes the same readings (`is_lock_name`, `pen_holder`, `is_conflict_copy`), and
a disagreement between the two is the desktop and the browser each believing a
different person holds the pen. Change one, change both, and the cases in both
test files.

### The calendar's database says which version it is

`rc_schema.sql` stamps `rc_settings.schema_version` as its **last** statement
(so a run that stopped half way does not claim to have finished), and
`SCHEMA_VERSION` in `core/rc.js` is the number this build expects.
`rc.schemaStatus()` compares them at sign-in: an administrator whose database
is behind sees a banner naming `migrate.sql` then `rc_schema.sql`; a member is
told some changes may be refused; a page older than its database is told to
reload. **Raise both together** whenever the calendar's schema changes shape —
`tools/test_sql.js` fails when they disagree.

### Only the calendar reports its failures, and only from named places

`rc.reportError(area, err)` writes a row to `rc_client_errors`, once per
message per page and at most twenty, and never throws; Organisation → Problems
lists them. It is called from specific call sites — a tab that failed to load,
an offline outcome the server refused, a look-ahead read that went wrong —
and **never from a global error handler**: an error thrown anywhere in the page
can carry plan text, and plan data must never reach Supabase. The table is
insert-own-row, admin-read, no update or delete; clearing goes through
`rc_clear_client_errors()`.

### The command menu adds no behaviour

`ui/command_menu.js` (mod+K, and "Go to…" at the top of the sidebar) lists
every dock pane from `TITLES`, the view and edit actions, every object type
from `TYPES`, the themes and the workspace switch. Each entry calls the command
a menu, button or shortcut already calls; a new pane appears in it by being in
`TITLES`. Sidebar sections fold on their heading (remembered per browser), and
a folded section never hides the open pane.

### Every calendar table gets the same behaviour, without asking

`ui/rc_table.js` runs over whatever a calendar tab draws (a `MutationObserver`
in `ui/rc.js`): the header stays in view on a long table, a heading click or
Enter sorts (numbers as numbers, blanks last, `aria-sort` set), and a heading
over a column of `.rc-num` cells aligns with them. A table marked
`data-csv="<name>"` gets an Export CSV button holding exactly the rows on
screen. Grids whose position is their meaning (`la-grid`, `rc-pto-grid`,
`rc-huddle`, `rc-resources`), tables with a spanning cell and tables marked
`data-plain` are left alone. Do not add a sort or an export to one table by
hand.

### An empty screen names its next step

`ui/canvas_hint.js` reads what the renderer just laid out (`RENDER_DONE`) and,
when nothing was, says whether the plan is empty or everything is hidden by a
date range, a hiding filter or hidden objects — with the command that ends it.
Elsewhere, `emptyState({ action })` carries the step. A pane asks for another
pane with `goToPane()` (`EV.PANE_OPEN`) and a calendar tab asks for another tab
with `goToTab()` (`EV.RC_SHOW_TAB`), because neither can import its router.

### Touch decides the calendar's sizes, not width

A tablet in landscape is wider than the 900px narrow rules, so on
`(pointer: coarse)` every calendar control reaches 44px and inputs use 16px text
(smaller zooms the page on focus). In landscape the meeting splits into the
question and the answer side by side. `smoke_calendar.js` drives an emulated
tablet in both orientations.

### Announce what changes; never rely on colour alone

The toast host is a polite live region that exists before anything is put
in it (a failure toast is an alert); the autosave state is announced and the
rest of the status bar is not, because it follows the pointer. Clickable status
bar items are keyboard buttons. A cancelled look-ahead day is struck through as
...
`run_tests.js` runs the suites side by side, in lanes: suites that write the
same directory (`test_dist` and `smoke_desktop` both assemble `dist-desktop/`)
share a lane and run in order. `tools/lib/clock.js` shifts `Date` — in Node and,
before anything else, in every page — to start on a pinned Wednesday afternoon,
because the calendar is about days and a suite reading the wall clock was a
different suite on a Saturday. Time still passes; only the starting instant is
fixed. `CX_TEST_NOW` picks another instant and `CX_TEST_NOW=real` unpins it; CI
runs the calendar a second time as though it were a Monday.
